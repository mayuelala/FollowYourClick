    ...
    *_, func_args = inspect.getargvalues(inspect.currentframe())
    ...
    for model_idx, (config_key, model_config) in enumerate(list(config.items())):
        ...
        for motion_module in motion_modules:
            ...
            motion_module_lora_path = model_config.get("motion_module_lora_path", '')
            ...
            if 'state_dict' in motion_module_state_dict: 
                ...
                missing, unexpected = pipeline.unet.load_state_dict(new_motion_module_state_dict, strict=False)
            else:
                missing, unexpected = pipeline.unet.load_state_dict(motion_module_state_dict, strict=False)
            # assert len(unexpected) == 0
            print (f'unexpected: {unexpected}')
            
            # motion_module_lora_state_dict = torch.load(motion_module_lora_path, map_location="cpu")
            # if "global_step" in motion_module_lora_state_dict: func_args.update({"global_step": motion_module_lora_state_dict["global_step"]})
            # if 'state_dict' in motion_module_lora_state_dict: 
            #     new_motion_module_lora_state_dict = {}
            #     for k,v in motion_module_lora_state_dict['state_dict'].items():
            #         if 'camera' not in k: contntinue
            #         new_motion_module_lora_state_dict[k.replace('module.', '')] = v
            #     missing, unexpected = pipeline.unet.load_state_dict(new_motion_module_lora_state_dict, strict=True)
            # else:
            #     missing, unexpected = pipeline.unet.load_state_dict(motion_module_lora_state_dict, strict=False)
            # assert len(unexpected) == 0
            ...
            path, alpha = motion_module_lora_path, 1.0
            print(f"load motion LoRA from {path} {alpha}")
